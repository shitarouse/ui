...
function NeverLose:CreateElementKeybind(Element, ElementType, GetValue, SetValue, Callback, FlagId, DisplayName, Signal, SliderInfo)
	...
	BindMenu.Size = UDim2.new(0, 200, 0, 0);
	...
	local function rowLabel(text)
		...
		l.Name = NeverLose.RandomString();
		l.Parent = BindMenu;
		l.BackgroundTransparency = 1;
		l.Position = UDim2.new(0, 12, 0, yOffset);
		l.Size = UDim2.new(0, 120, 0, 24);
		l.ZIndex = 402;
		l.Font = Enum.Font.GothamBold;
		l.Text = text;
		l.TextColor3 = Color3.fromRGB(255, 255, 255);
		l.TextSize = 13.000;
		l.TextTransparency = 0.350;
	...
	KeyBox.Position = UDim2.new(1, -38, 0, 16);
	...
	KeyText.TextTransparency = 0.500;
	KeyText.TextXAlignment = Enum.TextXAlignment.Center;

	local function updateKeySize()
		local s = TextService:GetTextSize(KeyText.Text, KeyText.TextSize, KeyText.Font, Vector2.new(math.huge, math.huge));

		NeverLose.PlayAnimate(KeyBox, SlowyTween, {
			Size = UDim2.new(0, math.max(45, s.X + 12), 0, 18)
		});
	end;
	...
	local RefreshMode;
	local ModeToggleBtn, ModeHoldBtn;

	if ElementType ~= "Button" then
		rowLabel("Mode");

		local ModeBox = Instance.new("Frame");
		local ModeBoxCorner = Instance.new("UICorner");
		local ModeBoxStroke = Instance.new("UIStroke");
		local ModeHighlight = Instance.new("Frame");
		local ModeHighlightCorner = Instance.new("UICorner");

		ModeBox.Name = NeverLose.RandomString();
		ModeBox.Parent = BindMenu;
		ModeBox.AnchorPoint = Vector2.new(1, 0.5);
		ModeBox.BackgroundColor3 = Color3.fromRGB(26, 28, 36);
		ModeBox.BorderSizePixel = 0;
		ModeBox.ClipsDescendants = true;
		ModeBox.Position = UDim2.new(1, -12, 0, yOffset + 12);
		ModeBox.Size = UDim2.new(0, 100, 0, 18);
		ModeBox.ZIndex = 402;

		ModeBoxCorner.CornerRadius = UDim.new(0, 4);
		ModeBoxCorner.Parent = ModeBox;

		ModeBoxStroke.Transparency = 0.650;
		ModeBoxStroke.Color = Color3.fromRGB(45, 48, 58);
		ModeBoxStroke.Parent = ModeBox;

		ModeHighlight.Name = NeverLose.RandomString();
		ModeHighlight.Parent = ModeBox;
		ModeHighlight.BackgroundColor3 = NeverLose.AccentColor;
		ModeHighlight.BorderSizePixel = 0;
		ModeHighlight.Position = UDim2.new(0, 0, 0, 0);
		ModeHighlight.Size = UDim2.new(0.5, 0, 1, 0);
		ModeHighlight.ZIndex = 402;

		ModeHighlightCorner.CornerRadius = UDim.new(0, 4);
		ModeHighlightCorner.Parent = ModeHighlight;

		ModeHighlight.BackgroundColor3 = NeverLose.AccentColor;

		local function makeSeg(text, xscale)
		...
		ModeToggleBtn = makeSeg("Toggle", 0);
		ModeHoldBtn = makeSeg("Hold", 0.5);

		RefreshMode = function()
			if KeybindData.Mode == "Hold" then
				NeverLose.PlayAnimate(ModeHighlight, SlowyTween, { Position = UDim2.new(0.5, 0, 0, 0) });
				NeverLose.PlayAnimate(ModeHoldBtn, SlowyTween, { TextTransparency = 0 });
				NeverLose.PlayAnimate(ModeToggleBtn, SlowyTween, { TextTransparency = 0.500 });
			else
				NeverLose.PlayAnimate(ModeHighlight, SlowyTween, { Position = UDim2.new(0, 0, 0, 0) });
				NeverLose.PlayAnimate(ModeToggleBtn, SlowyTween, { TextTransparency = 0 });
				NeverLose.PlayAnimate(ModeHoldBtn, SlowyTween, { TextTransparency = 0.500 });
			end;
		end;

		ModeToggleBtn.MouseButton1Click:Connect(LPH_NO_VIRTUALIZE(function()
			KeybindData.Mode = "Toggle";
			RefreshMode();
		end));

		ModeHoldBtn.MouseButton1Click:Connect(LPH_NO_VIRTUALIZE(function()
			KeybindData.Mode = "Hold";
			RefreshMode();
		end));
	...
	if ElementType == "Slider" and SliderInfo then
		...
		SContainer.AnchorPoint = Vector2.new(1, 0.5);
		SContainer.BackgroundTransparency = 1;
		SContainer.Position = UDim2.new(1, -12, 0, yOffset + 12);
		SContainer.Size = UDim2.new(0, 112, 0, 18);
		...
		ValueFrame.Size = UDim2.new(0, valW, 0, 18);
		...
		SlideMain.Size = UDim2.new(1, -(valW + 8), 0, 18);
	...
	RemoveIcon.AnchorPoint = Vector2.new(1, 0.5);
	RemoveIcon.BackgroundTransparency = 1;
	RemoveIcon.AutoButtonColor = false;
	RemoveIcon.Position = UDim2.new(1, -12, 0, 16);
	RemoveIcon.Size = UDim2.new(0, 18, 0, 18);
	RemoveIcon.ZIndex = 403;
	RemoveIcon.FontFace = NeverLose.BuiltInBold;
	RemoveIcon.Text = "trash-can";
	RemoveIcon.TextColor3 = Color3.fromRGB(255, 102, 105);
	RemoveIcon.TextSize = 15.000;
	RemoveIcon.TextTransparency = 0.250;

	local totalHeight = (yOffset > 34) and (yOffset + 8) or 44;
	...
	RemoveIcon.MouseButton1Click:Connect(LPH_NO_VIRTUALIZE(function()
		KeybindData.Key = nil;
	...
	NeverLose:AddSignal(RemoveIcon.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
		NeverLose.PlayAnimate(RemoveIcon, SlowyTween, {
			TextTransparency = 0
		});
	end)));

	NeverLose:AddSignal(RemoveIcon.MouseLeave:Connect(LPH_NO_VIRTUALIZE(function()
		NeverLose.PlayAnimate(RemoveIcon, SlowyTween, {
	...
	local function OpenMenu()
		...
		if px + 200 > viewport.X then
			px = ePos.X - 200 - 8;
		end;

		local py = ePos.Y + (eSize.Y / 2) - (totalHeight / 2);
		py = math.clamp(py, 8, math.max(8, viewport.Y - totalHeight - 8));
...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		...
		end, EmptyFunction, Config.Flag, Config.Flag or "Keybind", Signal);
...
function NeverLose:CreateWindow(Config)
	...
	function Window:Watermark()
		...
		NeverLose:CreateBlurModule(Watermark, WatermarkBlur);
		...
		Watermark.BackgroundTransparency = 0.100
	...
	function Window:KeyList()
		...
		local function makeTumbler(parent)
			...
			return {
				set = function(on)
					if on then
						NeverLose.PlayAnimate(t, SlowyTween, { BackgroundColor3 = NeverLose.AccentColor });
						NeverLose.PlayAnimate(circle, SlowyTween, { Position = UDim2.new(0.7, 0, 0.5, 0), BackgroundTransparency = 0 });
					else
						NeverLose.PlayAnimate(t, SlowyTween, { BackgroundColor3 = Color3.fromRGB(10, 13, 21) });
						NeverLose.PlayAnimate(circle, SlowyTween, { Position = UDim2.new(0.3, 0, 0.5, 0), BackgroundTransparency = 0.500 });
					end;
				end,
			};
		end;
		...
		local function createEntry(kb)
			...
			if hasTumbler then
				...
				table.insert(e.Cards, { TumblerCard, TumblerStroke, TumblerShadow, 34, TumblerBlur });
			end;

			local InfoCard, InfoStroke, InfoShadow, InfoBlur = makeCard(Row, 2);
			InfoCard.AutomaticSize = Enum.AutomaticSize.X;

			table.insert(e.Cards, { InfoCard, InfoStroke, InfoShadow, 0, InfoBlur });
		...
		local function animateIn(e)
			for _, c in next , e.Cards do
				NeverLose.PlayAnimate(c[1], VSlowTween, { Size = UDim2.new(0, c[4], 0, 28), BackgroundTransparency = 0.200 });
				NeverLose.PlayAnimate(c[2], SlowyTween, { Transparency = 0.650 });
				c[3]:Render(true);
				c[5]:SetValue(true);
			end;

			NeverLose.PlayAnimate(e.Name, SlowyTween, { TextTransparency = 0.100 });
			NeverLose.PlayAnimate(e.Key, SlowyTween, { TextTransparency = 0.150 });

			if e.Value then
				NeverLose.PlayAnimate(e.Value, SlowyTween, { TextTransparency = 0.150 });
			end;
		end;

		local function animateOut(e)
			for _, c in next , e.Cards do
				NeverLose.PlayAnimate(c[1], VSlowTween, { Size = UDim2.new(0, c[4], 0, 0), BackgroundTransparency = 1 });
				NeverLose.PlayAnimate(c[2], SlowyTween, { Transparency = 1 });
				c[3]:Render(false);
				c[5]:SetValue(false);
			end;

			NeverLose.PlayAnimate(e.Name, SlowyTween, { TextTransparency = 1 });
			NeverLose.PlayAnimate(e.Key, SlowyTween, { TextTransparency = 1 });

			if e.Value then
				NeverLose.PlayAnimate(e.Value, SlowyTween, { TextTransparency = 1 });
			end;
		...
		local function showPanel()
			...
			NeverLose.PlayAnimate(HeaderCard, SlowyTween, { BackgroundTransparency = 0.200 });
			NeverLose.PlayAnimate(HeaderStroke, SlowyTween, { Transparency = 0.650 });
			HeaderShadow:Render(true);
			NeverLose.PlayAnimate(HeaderTitle, SlowyTween, { TextTransparency = 0 });
			NeverLose.PlayAnimate(HeaderIcon, SlowyTween, { ImageTransparency = 0 });
		end;
		...
		local function hidePanel()
			...
			NeverLose.PlayAnimate(HeaderCard, SlowyTween, { BackgroundTransparency = 1 });
			NeverLose.PlayAnimate(HeaderStroke, SlowyTween, { Transparency = 1 });
			HeaderShadow:Render(false);
			NeverLose.PlayAnimate(HeaderTitle, SlowyTween, { TextTransparency = 1 });
			NeverLose.PlayAnimate(HeaderIcon, SlowyTween, { ImageTransparency = 1 });
		end;
		...
		local function refresh()
			...
			for id, kb in next , (NeverLose.ElementKeybinds or {}) do
				if kb.Key then
					count = count + 1;
					...
					updateEntry(e, kb);
				elseif entries[id] then
					animateOut(entries[id]);
					entries[id] = nil;
				end;
			end;
			...
			for _, e in next , entries do
				if e.Row and e.Row.AbsoluteSize.X > maxW then
					maxW = e.Row.AbsoluteSize.X;
				end;
			end;

			NeverLose.PlayAnimate(HeaderCard, SlowyTween, { Size = UDim2.new(0, maxW, 0, shown and 30 or 0) });
		end;
...
function NeverLose:CreateLogger()
	...
	function Logging.new(IconStr: string , Message: string , Duration: number)
		...
		local LogImage = nil;
		local __logIcon = tostring(IconStr);
		if string.match(__logIcon, "^%d+$") then
			Icon.Text = "";
			LogImage = Instance.new("ImageLabel");
			LogImage.Name = NeverLose.RandomString();
			LogImage.Parent = LogFrame;
			LogImage.BackgroundTransparency = 1;
			LogImage.BorderSizePixel = 0;
			LogImage.Position = Icon.Position;
			LogImage.Size = Icon.Size;
			LogImage.ZIndex = 133;
			LogImage.Image = "rbxassetid://" .. __logIcon;
			LogImage.ImageColor3 = Color3.fromRGB(223, 223, 223);
		...
		task.delay(0.15,LPH_NO_VIRTUALIZE(function()
			...
			if LogImage then
				NeverLose.PlayAnimate(LogImage , SlowyTween , {
